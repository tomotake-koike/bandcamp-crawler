#!/usr/bin/env python3
import json, os, requests, re
from bs4 import BeautifulSoup

start_point = re.compile( "^var\s+TralbumData\s*=\s*{" )
end_point = re.compile( "^\}" )
comment_part = re.compile( "(\/\/\s.+)$" )
string_bonding = re.compile( '("\s*\+\s*")' )
json_line = re.compile( "^\s*([^:\s]+)\s*:(.+)$" )
path_query = re.compile( "[\/\*\$\;\<\>]" )

def download( url ):
    TralbumData = ""

    m=re.search( '\/\/(\w+)\.', url )
    group = m.group(1)
    gpath = path_query.sub( "_", group )
    if not os.path.isdir( gpath ) : os.mkdir( gpath )

    with requests.get( url ) as res:

        soup = BeautifulSoup( res.content, "html.parser" )
        scripts = soup.find_all("script")
        
        for l in scripts[10].string.splitlines() :
            # l = l.decode()
            if start_point.search( l ) : TralbumData = "{"
            if TralbumData :
                l = comment_part.sub( "", l )
                if end_point.search( l ) :
                    TralbumData += "}"
                    break
                m = json_line.search( l )
                if m :
                    TralbumData += '"' + m.group( 1 ) + '": ' + string_bonding.sub( "", m.group( 2 ) )

    j = json.loads( TralbumData )
    if j:
        artist = "anonimous"
        if j["artist"] : artist = j["artist"]
        path = gpath + "/" + path_query.sub( "_", artist )
        if not os.path.isdir( path ) : os.mkdir( path )
    
        album = "no title"
        if j["packages"]:
            if j["packages"][0]["album_title"] : album = j["packages"][0]["album_title"]
        else:
            if j["current"] and j["current"]["title"] : album = j["current"]["title"]

        out_dir = path + "/" + path_query.sub( "_", album )
        if not os.path.isdir( out_dir ) : os.mkdir( out_dir )
    
#        print( "Downloading.. : " + path_query.sub( "", artist + "/" + album ) )

        for t in j["trackinfo"]:
            track_num = 0
            if t['track_num'] : track_num = t['track_num']
            out =  out_dir + "/" + path_query.sub( "_", "{0:02d}_".format( track_num ) + t['title'] + ".mp3" )
            if not os.path.exists( out ):
                if t['file']:
                    print( "Downloading ... : " + out )
                    mp3_url = t['file']['mp3-128']
                    with requests.get( mp3_url ) as res:
                        with open( out, 'wb' ) as of:
                            of.write( res.content )




if __name__ == '__main__':
    
    url = os.sys.argv[ 1 ]
    album_href = re.compile( '^\s*\<a\shref=\"(\/(track|album)[^\"]+)\"' )
    if re.search( "\/(album|track)\/" , url ) :
        download( url )
    else:
       with requests.get( url ) as res:
           for l in res.content.splitlines() :
               l = l.decode()
               m = album_href.search( l )
               if m:
                   download( url + m.group( 1 ) )

                   
               
